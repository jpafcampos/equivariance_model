...
class RotEquivariantResvit(nn.Module):
    ...
    def __init__(self, backbone, group_elements, num_class, dim, depth, heads, mlp_dim):
        super(RotEquivariantResvit, self).__init__()
        self.backbone = backbone
        self.dim = dim
        self.heads = heads
        self.dim_head = dim//heads
        group = g_selfatt.groups.E2(num_elements=group_elements)
        attention_layer = GroupLocalSelfAttention(group = group,
                                       in_channels = dim,
        ...
        #self.proj    = nn.Conv2d(in_channels=181, out_channels=dim, kernel_size=3, padding=1)
        self.proj    = nn.Conv2d(in_channels=5792, out_channels=dim, kernel_size=1, stride=1)
...
#backbone = Wide_ResNet(52, 1, 0.3, initial_stride=2, N=4, f=True, r=0, num_classes=1)
#backbone = g_resnet.ResNet50()
#model = RotEquivariantResvit(backbone, 4, 5, 768, 1, 2, 768)
